...
public class PermissionManagementTest extends AbstractResourceServerTest {
    ...
    public void testCreatePermissionTicketWithResourceId() throws Exception {
        ResourceRepresentation resource = addResource("Resource A", true);
        PermissionResponse response = getAuthzClient().protection("marta", "password").permission().create(new PermissionRequest(resource.getId(), Collections.emptySet()));
        assertNotNull(response.getTicket());
        assertFalse(getAuthzClient().protection().permission().findByResource(resource.getId()).isEmpty());
    }

    @Test
    public void testCreatePermissionTicketWithScopes() throws Exception {
        ResourceRepresentation resource = addResource("Resource A", true, "ScopeA", "ScopeB", "ScopeC");
        PermissionResponse response = getAuthzClient().protection("marta", "password").permission().create(new PermissionRequest(resource.getId(), new HashSet<>(Arrays.asList("ScopeA", "ScopeB", "ScopeC"))));
    ...
    public void testDeleteScopeAndPermissionTicket() throws Exception {
        ...
        PermissionResponse response = getAuthzClient().protection("marta", "password").permission().create(permissionRequest);
        assertNotNull(response.getTicket());
        assertEquals(3, getAuthzClient().protection().permission().findByResource(resource.getId()).size());

        AuthorizationResource authorization = getClient(getRealm()).authorization();
        ResourceScopesResource scopes = authorization.scopes();
        ScopeRepresentation scope = scopes.findByName("ScopeA");

        List permissions = getAuthzClient().protection().permission().findByScope(scope.getId());
        ...
        assertTrue(getAuthzClient().protection().permission().findByScope(scope.getId()).isEmpty());
        assertEquals(0, getAuthzClient().protection().permission().findByResource(resource.getId()).size());
    }

    @Test
    public void testRemoveScopeFromResource() throws Exception {
        ResourceRepresentation resource = addResource("Resource A", true, "ScopeA", "ScopeB");
        ...
        List permissions = getAuthzClient().protection().permission().findByScope(removedScope.getId());
        assertFalse(permissions.isEmpty());
        ...
        authorization.resources().resource(resource.getId()).update(resource);
        permissions = getAuthzClient().protection().permission().findByScope(removedScope.getId());
        assertTrue(permissions.isEmpty());

        ScopeRepresentation scopeB = scopes.findByName("ScopeB");
        permissions = getAuthzClient().protection().permission().findByScope(scopeB.getId());
        assertFalse(permissions.isEmpty());
    }
    ...
    public void testTicketNotCreatedWhenResourceOwner() throws Exception {
        ...
        List permissions = getAuthzClient().protection().permission().findByResource(resource.getId());
        assertTrue(permissions.isEmpty());

        response = getAuthzClient().protection("kolo", "password").permission().create(new PermissionRequest(resource.getId(), Collections.emptySet()));
        assertNotNull(response.getTicket());
        permissions = getAuthzClient().protection().permission().findByResource(resource.getId());
        assertFalse(permissions.isEmpty());
        assertEquals(1, permissions.size());
    }
    ...
    public void testPermissionForTypedScope() throws Exception {
        ...
        PermissionResponse response = getAuthzClient().protection("kolo", "password").permission().create(permissionRequest);
    ...
    private void assertPersistence(PermissionResponse response, ResourceRepresentation resource, String... scopeNames) throws Exception {
        ...
        assertEquals(expectedPermissions, tokenPermissions.size());
        ...
        while (permissionIterator.hasNext()) {
            ...
            if (tickets.stream().filter(representation -> representation.getId().equals(resourcePermission.getTicket())).count() == 1) {
    ...
    @Test
    public void failInvalidResource() throws Exception {
        try {
            ...
            assertTrue(e.getMessage().contains("invalid_resource_id"));
        }
        try {
