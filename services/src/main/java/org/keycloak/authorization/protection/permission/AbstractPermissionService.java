...
public class AbstractPermissionService {
            ...
            if (resource != null) {
                ...
                if (scope == null && resource.getType() != null) {
                    scope = resourceStore.findByType(resource.getType(), resourceServer.getId()).stream()
                            .filter(baseResource -> baseResource.getOwner().equals(resource.getResourceServer().getClientId()))
    ...
    private String createPermissionTicket(List<PermissionRequest> request) {
        ...
        List<PermissionTicketToken.ResourcePermission> permissions = verifyRequestedResource(request).stream().flatMap(resource -> {
            ...
            if (isPersistentTicket(resource)) {
                if (scopes.isEmpty()) {
                    perms.add(new PermissionTicketToken.ResourcePermission(getOrCreateTicket(ticketStore, resource, null)));
                } else {
                    for (ScopeRepresentation scope : scopes) {
                        perms.add(new PermissionTicketToken.ResourcePermission(getOrCreateTicket(ticketStore, resource, scope)));
                    }
                }
            } else {
    ...
    private String getOrCreateTicket(PermissionTicketStore ticketStore, ResourceRepresentation resource, ScopeRepresentation scope) {
        Map<String, String> filter = new HashMap<>();

        filter.put(PermissionTicket.RESOURCE, resource.getId());

        if (scope != null) {
            filter.put(PermissionTicket.SCOPE, scope.getId());
        }

        filter.put(PermissionTicket.OWNER, resource.getOwner().getId());
        filter.put(PermissionTicket.REQUESTER, identity.getId());

        List<PermissionTicket> existing = ticketStore.find(filter, resourceServer.getId(), -1, -1);

        if (existing.isEmpty()) {
            return ticketStore.create(resource.getId(), scope == null ? null : scope.getId(), identity.getId(), resourceServer).getId();
        }

        return existing.get(0).getId();
    }

    private boolean isPersistentTicket(ResourceRepresentation resource) {
        return Boolean.TRUE.equals(resource.getOwnerManagedAccess()) && !identity.getId().equals(resourceServer.getClientId()) && !identity.getId().equals(resource.getOwner().getId());
    }
}
